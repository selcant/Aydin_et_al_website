...
all.ids <- read_tsv(here("_data/Corrected_PBID_SampleMatch_Key_v4.tsv")) %>%
  mutate(sampleid.esc_prot = gsub("rep1", "repA", sampleid.esc_prot))
correct.atac.ids <- read_tsv(here("_data/ATAC_sampleids_corrected.tsv"))
...
# pathVar prep
keg.annots.symbol <- pathVar::makeDBList(here("../pQTL_website/_data/kegg.annots.symbol.txt"))
keg.annots.prot <- pathVar::makeDBList(here("../pQTL_website/_data/kegg.annots.prot.txt"))
keg.annots.gene <- pathVar::makeDBList(here("../pQTL_website/_data/kegg.annots.gene.txt"))
...
genes_2c <- read_xlsx( path = here("_data","Hendrickson_2017_tableS8_2clike_gene_list.xlsx"), sheet = 2) %>% 
...
# for LOLA
regionDB = loadRegionDB(here("../pQTL_website/_data/LOLA/nm/t1/resources/regions/LOLACore/mm10/"))
...
```{r, fig.align='center', echo=FALSE, fig.cap="Experimental overview."}
knitr::include_graphics(here("figures/fig1a.png"))
```
...
Below are the filtered, normalized abundances of `r formatC(ncol(expr.esc_prot), big.mark=",")` proteins across `r nrow(expr.esc_prot)` DO mESC lines.  
...
## Chromatin accessibility (ATAC-seq) and gene expression (RNA-seq)

Chromatin accessibility and gene expression data sets area obtained from Skelly et al,. 2020. There are `r formatC(nrow(atac.counts), big.mark=",")` ATAC-seq peaks measured across `r ncol(atac.counts)-3` DO mESC lines where the peaks are normalized for library size using `edgeR cpm` function. Here, I am using the `ComBat` function from `sva` package to additionally correct for the effects of library prep done in two separate batches. Next, I add functional annotations using the `annotatePeak` function from `ChIPseeker` R package. We have transcript abundance measurements for `r formatC(ncol(expr.esc_rna), big.mark=",")` genes across `r nrow(expr.esc_rna)` DO mESC lines. The raw counts were obtained using the RNA-seq analysis pipeline described in Skelly et al., 2020 using paired-end 75bp reads. Briefly, the total raw counts per gene were retrieved using EMASE, upper quartile normalized and corrected for batch effects due to different library preps using `ComBat` function from `sva` R package.   

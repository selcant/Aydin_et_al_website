...
```{r Table_prot_atac_cor1, fig.cap = "Data used in Figure 3A to generate the genomic regions in the circos plot."}
...
```{r Table_prot_atac_cor2, fig.cap = "Data used in Figure 3A to add ID1 gene annotation."}
...
prot_rna_sample_cor_df %>%
  ...
  annotate( "text",
            x = median_cor_rna_prot$med_r + 0.12,
            label = paste0("Median = ",round(median_cor_rna_prot$med_r,2)), 
            y = 13,
            size = 5
  )+
  xlab("Cell line correlation") +
  ylab("Count") +
  #scale_fill_viridis_d(direction = -1, limits = c("ns", "p < 0.05", "p < 0.005", "p < 5e-10")) +
...
prot_rna_sample_cor_df %>%
  mutate( sampleid_prot = gsub("_protein","",protein_sample),
          sampleid_rna = gsub("_rna","", rna_sample)) %>%
  filter( sampleid_rna == sampleid_prot) %>%
  select( `Sample id` = sampleid_prot, 
          `Correlation` = r) %>% 
  mutate_if(is.numeric, round ,2) %>% 
  create_dt()
...
# }
# 
# save(sample_cor, file = here("_data","rna_prot_sample_cor_perm.RData"))


load(here("../pQTL_website/_data","rna_prot_sample_cor_perm.RData"))
...
prot_rna_gene_cor_df %>%
  ...
  mutate( Group = case_when(   ( Correlation < 0 & p_adj < 0.05) ~ "negative",
                               ( Correlation > 0 & p_adj < 0.05) ~ "positive",
                               ( abs(Correlation) <= 0.05 ) ~ "low",
                               ( abs(Correlation) > 0.05 & p_adj >= 0.05) ~ "none"
                              )
  ) %>% 
  ggplot() +
  aes(x = Correlation) +
  ...
  ylab("")+
  scale_fill_manual(limits = c("low", "none", "negative", "positive"), values = c(y_colors[3],"gray",y_colors[2],y_colors[1]) ) +
  theme_pubclean(base_size = 18, base_family = "Poppins")+
  geom_vline( aes(xintercept = median(Correlation)), linetype = 2, col = "black", size = 1) +
  annotate("text",
    x = median(prot_rna_gene_cor_df$cor, na.rm = T) -0.25, y = 135,
    label = paste0("Median = \n", formatC(median(prot_rna_gene_cor_df$cor, na.rm = T), digits = 1, format ="f")),
    size = 6,
    family = "Poppins"
  )+
  xlim(-0.5, 1)+
...
ora_all_corr %>%
  ...
  left_join(., prot_rna_gene_cor_df, by = c("intersection" = "mgi_symbol")) -> data
...
data %>%
  ...
  theme(
    ...
    axis.text.y = element_text(size = 13, color = (med.cor$y_col))
  ) +
...
```{r Figure3C_data, warning=FALSE, message=FALSE, fig.cap ="Correlation values used in plotting Figure 3C with groups labeled."}

prot_rna_gene_cor_df %>% 
  mutate( 
    Group = case_when(
      (cor < 0 & p_adj < 0.05) ~ "Negative corr.",
      (cor > 0 & p_adj < 0.05) ~ "Positive corr.",
      ( abs(cor) < 0.05) ~ "No corr."
    )
    ) %>% 
   select( 

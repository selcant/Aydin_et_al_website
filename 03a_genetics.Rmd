...
peaks.esc_prot %>%
  ...
  filter(!is.na(mgi_symbol)) -> peaks.esc_prot_annotated
...
ggplot()+
  ...
  geom_point(data = peaks.esc_prot_annotated %>% 
            filter( lod > 7.5), 
  ...
  theme( axis.text = element_text(size = 10),
          ...
          panel.grid.major.y = element_blank()) -> pqtl_plot
...
```{r Data_for_4A, fig.cap="Significant pQTL details plotted in Figure 4A.", warning=FALSE, message=FALSE}
...
Significant pQTL (LOD > 7.5) are listed with gene annotations including ensembl protein and gene ID, chromosomal, gene start and end location. In addition, QTL peak details like the location of the peak in centi Morgan and base pairs, as well QTL type ( within 10Mb of the midpoint is classified as local) are listed for each individual pQTL. 
...
shared.qtl.eff.corrs.df %>%
  mutate(`Significance` =  ifelse(p_adj < 0.1, "FDR < 0.1", "ns")) %>% 
  ggplot() +
  aes(y = cor, col = Significance, fill = Significance) +
...
```{r Figure4B_data, fig.cap="Correlation values used in plotting in Figure 4B", warning=FALSE, message=FALSE}
...
bspry_effects %>% 
  ...
  mutate( Strain = case_when( Strain == "A" ~ "A/J",
                              ...
                              Strain == "G" ~ "PWK",
                              Strain == "H" ~ "WSB")) %>% 
  mutate( QTL = factor(QTL, levels =c("esc_atac","esc_rna","esc_prot"))) %>% 
  ggplot()+
  aes( x = Strain,
       y = value, 
       col = QTL,
...
bspry_qtl_scans %>% 
  ...
  ylab( "LOD score")+
  labs(col = "QTL type")+
  geom_segment( x = bspry_qtl$gene_start, xend = bspry_qtl$gene_end , y = 0, yend = 1, col = "black", size = 2) +
  annotate( "text", x= bspry_qtl$midpoint, y = 2, label ="BSPRY", size =6) -> bspry_lod_plot
...
# LOD plots
tfcp2l1_qtl_scans %>% 
  filter( chr == tfcp2l1_qtl$peak_chr) %>% 
  pivot_longer( cols = c("caqtl","eqtl","pqtl"), names_to = "qtl_type", values_to = "lod") %>% 
  mutate( qtl_type = factor( qtl_type, levels = c("caqtl","eqtl","pqtl"))) %>% 
  ggplot()+
    aes( 
  ...
  ylab( "LOD score")+
  labs(col = "QTL type")+
  geom_segment( x = tfcp2l1_qtl$gene_start, xend = tfcp2l1_qtl$gene_end , y = 0, yend = 1, col = "black", size = 2) +
  annotate( "text", x= tfcp2l1_qtl$midpoint, y = 2, label ="TFCP2L1", size =6) +
...
```{r Figure4C_data2, fig.cap="Haplotype effects used to generate Figure 4C."}
...
```{r Figure_4D, warning=FALSE, message=FALSE, fig.cap = "Figure 4D: Histogram depicting the number of total distant pQTL forming hotspots across the genome.", fig.width=7, fig.height=3}
...
rpa_effects %>% 
  column_to_rownames("mgi_symbol") %>%  t() -> rpa_effects_mat
...
rpa_effects %>% 
  ...
  mutate( Strain = case_when( Strain == "A" ~ "A/J",
                              ...
                              Strain == "F" ~ "CAST",
                              Strain == "G" ~ "PWK",
                              Strain == "H" ~ "WSB")) -> rpa_effects_plot_data

rpa_effects_plot_data %>% 
  ggplot()+
  aes( x = Strain,
       y = `pQTL effect`, 
       group= Protein, 
...
rpa_pqtl %>% 
  ...
  rename( `LOD score` = lod, `Marker Position (bp)` = pos_bp) -> rpa_pqtl_scans
...
rpa_pqtl_scans %>% 
  ...
  scale_color_manual( values = c( RColorBrewer::brewer.pal(6, "Blues")[3],
                                  RColorBrewer::brewer.pal(6, "Blues")[6],
                                  RColorBrewer::brewer.pal(6, "Blues")[4]))+
  xlab(paste0("Chr ",rpa_qtl$peak_chr[1]," location (bp)"))+
  ylab( "LOD score")+
  labs(col = "Protein")+
  geom_segment( x = rpa_qtl$gene_start[1], xend = rpa_qtl$gene_end[1] , y = 0, yend = 1, col = "black", size = 2) +
  annotate( "text", x= rpa_qtl$midpoint[1], y = 2, label ="italic(Rpa3)", parse = TRUE, size =6) +
...
```{r Figure4E_data_qtl_scans, fig.cap="QTL scans used in plotting LOD plots in Figure 4E."}
...
rpa_pqtl_scans %>% 
  select( Protein, 
          `LOD score` , 
          `Marker Position (bp)`) %>% 
  mutate(`LOD score` = formatC(`LOD score`, digits =2)) %>% 
  create_dt()
...
```{r Figure4E_data_effects, fig.cap="Haplotype effects plotted in Figure 4E."}
...
shared.kinship <- calc_kinship(threeway.shared.probs, type = "loco")
shared.covar <- merged.covar[threeway.shared.samples$top_muga, , drop = FALSE]
rownames(shared.covar) <- threeway.shared.samples$sampleid
...
for (i in 1:length((significant_pQTL_annot2$qtl_id))) {
  ...
  if( significant_pQTL_annot2$eff_stat[i] =="missing eQTL"){
    ...
    effs_rna <- scan1blup(
      ...
      kinship = shared.kinship[[this_chrom]], 
  ...
  #     kinship = shared.kinship[[this_chrom]], addcovar = shared.covar
  #   )
